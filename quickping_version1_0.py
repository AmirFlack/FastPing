...
class QuickPingApp:
    def __init__(self, root):
        self.root = root
        self.root.title("QuickPing")
        self.root.geometry("500x500")
        self.root.resizable(False, False)  # Fixed size window

        # Header frame for credit, contact, and GitHub links
        header_frame = ctk.CTkFrame(root, fg_color="transparent")
        header_frame.pack(pady=10, padx=10, fill="x")

        # Left side: credit and contact
        left_header = ctk.CTkFrame(header_frame, fg_color="transparent")
        left_header.pack(side="left")
        credit_label = ctk.CTkLabel(
            left_header, text="credits: kalin_9", text_color="black", font=("Helvetica", 14, "bold"), fg_color="transparent"
        )
        credit_label.pack(side="left", padx=(0, 20))
        contact_label = ctk.CTkLabel(
            left_header, text="Contact me", text_color="dodgerblue", font=("Helvetica", 14, "bold"), fg_color="transparent", cursor="hand2"
        )
        contact_label.pack(side="left")
        contact_label.bind("<Button-1>", lambda e: webbrowser.open("https://t.me/kalin"))

        # Right side: GitHub link
        github_label = ctk.CTkLabel(
            header_frame, text="GitHub", text_color="dodgerblue", font=("Helvetica", 14, "bold"), fg_color="transparent", cursor="hand2"
        )
        github_label.pack(side="right")
        github_label.bind("<Button-1>", lambda e: webbrowser.open("https://github.com/kalin"))

        # Main title
        title_label = ctk.CTkLabel(root, text="QuickPing", text_color="black", font=("Helvetica", 18, "bold"))
    ...
    def remove_ip(self, ip):
        ...
        update_ips_file(self.ips)
        # (Optional: Rearrange rows if needed)
